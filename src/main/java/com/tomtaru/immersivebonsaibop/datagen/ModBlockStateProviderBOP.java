...
public class ModBlockStateProviderBOP extends BlockStateProvider {

    public ModBlockStateProviderBOP(PackOutput output, ExistingFileHelper existingFileHelper) {
        super(output, ImmersiveBonsaiBOP.MODID, existingFileHelper);
    }
     ...
     */
    private void makeBonsai(BonsaiDataBOP.BonsaiType bonsaiType) {
        DeferredBlock<?> block = bonsaiType.bonsai();
        String name = block.getId().getPath();
        String woodType = bonsaiType.woodType();
        String texturePath = "biomesoplenty:block/"+ woodType + bonsaiType.plantType().getPlantType();;
